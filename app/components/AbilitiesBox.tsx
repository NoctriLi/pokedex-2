...
const AbilitiesBox = ({ abilities, type } : {abilities: any[], type: string}) => {
    ...
    <div className={styles.main}>
      ...
      <ul className={styles.list}>
        {abilities.map((ability, index) => {
          return <li className={styles.listItem} key={index}>{ability.ability.name }<br /> <span className={styles.hiddenAb}>{ (ability.is_hidden ? " (Hidden Ability)" : "")}</span></li>;
        })}
      </ul>
    </div>
  );
