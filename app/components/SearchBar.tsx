...
const SearchBar = () => {
  ...
  return (
    <nav className="absolute top-0  right-0 flex items-center justify-between w-full h-20 gap-2 p-2 bg-electric-l">
      ...
      <div
        className={`absolute top-20 right-0 flex flex-col max-w-xl h-fit bg-electric-l p-2 transition-opacity duration-300  gap-2 ${
          visible ? "w-full opacity-100" : "w-0 opacity-0 hidden"
        }`}
      >
        <input
          ...
          className={`${
            visible ? "w-full" : "w-0"
          } h-10 p-2 bg-fire-l shadow-inner shadow-black/30 placeholder:text-black/30 text-center`}
        />
        <Link
