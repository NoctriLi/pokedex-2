...
const StatsBox = ({ stats, type }: {stats: Stats[], type: string}) => {
    ...
    const total = stats.reduce((acc: number, stat: { base_stat: number; }) => acc + stat.base_stat, 0);
      ...
      <tbody >
        {stats.map((stat, i) => (
          <tr className={styles.row} key={i}>
            <td className="flex flex-col items-center justify-center capitalize text-lg">{formatStatName(stat.stat.name) + ": "} <br/>{ stat.base_stat}</td>
            <td className={styles[stat.stat.name]}>
                <p className={styles[stat.stat.name + "Bar"]} style={{width: widthPercentages[stat.stat.name] + "%"}}></p>
            </td>
          </tr>
        ))}
      </tbody>
